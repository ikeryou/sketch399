...
export class Visual extends Canvas {
  ...
  constructor(opt: any) {
    ...
    for(let i = 0; i < 2; i++) {
      const t = TexLoader.instance.get(Conf.instance.PATH_IMG + 't' + i + '.png');
    ...
    document.querySelectorAll('a').forEach((el) => {
      el.addEventListener('mouseenter', () => {
        Param.instance.isHover = true
      })
      el.addEventListener('mouseleave', () => {
        Param.instance.isHover = false
      })
    })
  ...
  protected _update(): void {
    ...
    const t = Param.instance.isHover ? 1 : 0
    Param.instance.hoverRate = t
